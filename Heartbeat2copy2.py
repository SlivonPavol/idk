...
disp.add_text("oxygen_num", "89", ((320/2)-50, 190), font=Beat_num_font, color=text_color)
...
def calculate_hr_and_spo2(data, prw_fps):
    ...
    if (first_10_seconds == False):
        ...
        disp.update_text("Beat_num", str(round(heart_rate_bpm * 60)))
        disp.update_text("oxygen_num", str(round(oxygen_percentage)))
        disp.display()
                ...
                for face_landmarks in processed_img.multi_face_landmarks:
                    ...
                    if elapsed_time >= 10:
                        if not first_10_seconds:
                            ...
                            if(heart_rate):
                                mean_oxygen_saturation = round(np.mean(oxygen_saturation))
                                mean_heart_rate = round(np.mean(heart_rate))
                                socketio.emit('heart_rate', {'value': mean_heart_rate})
                                socketio.emit('oxygen_saturation', {'value': mean_oxygen_saturation})
                                disp.update_text("Beat_num", str(mean_heart_rate))
                                disp.update_text("oxygen_num", str(mean_oxygen_saturation))
                                disp.display()

                                manage_csv_file('templates/data.csv',
                                                [mean_heart_rate, mean_oxygen_saturation])
                            heart_rate.clear()
